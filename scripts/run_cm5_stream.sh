...
echo "Starting CM5 headless stream profile=$DEVICE_PROFILE backend=$SMARTGLASSES_CAMERA_BACKEND scene=$SCENE_CAMERA connector=$SCENE_CAMERA_CONNECTOR eye_enabled=$ENABLE_EYE_CAMERA eye=$EYE_CAMERA host=$STREAM_HOST port=$STREAM_PORT out=$SMARTGLASSES_OUTDIR"
