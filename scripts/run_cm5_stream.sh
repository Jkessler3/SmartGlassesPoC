...
export SCENE_CAMERA_CONNECTOR="${SCENE_CAMERA_CONNECTOR:-CAM/DISP0}"
export SCENE_CAMERA="${SCENE_CAMERA:-0}"
export ENABLE_EYE_CAMERA="${ENABLE_EYE_CAMERA:-1}"
export EYE_CAMERA_CONNECTOR="${EYE_CAMERA_CONNECTOR:-CAM/DISP1}"
...
export STREAM_HOST="${STREAM_HOST:-0.0.0.0}"
export STREAM_PORT="${STREAM_PORT:-8000}"
...
echo "Starting CM5 headless stream profile=$DEVICE_PROFILE backend=$SMARTGLASSES_CAMERA_BACKEND scene=$SCENE_CAMERA connector=$SCENE_CAMERA_CONNECTOR eye_enabled=$ENABLE_EYE_CAMERA eye=$EYE_CAMERA eye_connector=$EYE_CAMERA_CONNECTOR host=$STREAM_HOST port=$STREAM_PORT out=$SMARTGLASSES_OUTDIR"
